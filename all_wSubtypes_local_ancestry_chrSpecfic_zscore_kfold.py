...
# table directories 
table_dir=f'/work/carrot-zhang/david/local_ancestry/tables/{sys.argv[1]}_LocalAncestry_{sys.argv[12]}_table.csv'
df_final=pd.read_csv(f'{table_dir}', delimiter=",", low_memory=False)
kf = KFold(n_splits=sys.argv[13])


#chr_dir=f'/work/carrot-zhang/david/local_ancestry/ancestryscore/{sys.argv[12]}.afr_amr_eur.msp_subject.tsv'
#df_chr=pd.read_csv(f'{chr_dir}', delimiter=" ", low_memory=False)

subs='snp'
res = [i for i in df_final.columns if subs in i]

# defining variables
txtpath = '/work/carrot-zhang/david/local_ancestry/txtResults' 
csvpath='/work/carrot-zhang/david/local_ancestry/results'

if not os.path.exists(csvpath):
    os.makedirs(csvpath)

if not os.path.exists(f'{csvpath}/{sys.argv[12]}'):
   os.makedirs(f'{csvpath}/{sys.argv[12]}')

if not os.path.exists(txtpath):
    os.makedirs(txtpath)

if not os.path.exists(f'{txtpath}/{sys.argv[12]}'):
   os.makedirs(f'{txtpath}/{sys.argv[12]}')
...
for train_index, test_index in kf.split(df_final[sys.argv[2].values):
   ...
   count=0
   for snp in snp_loc:
      filename=home+'/txtResults/'+f'{sys.argv[12]}/'+f'{sys.argv[3]}_All_LocalAncestry_summarys_{sys.argv[4]}_{sys.argv[12]}_{snp}_zscore_kfold.txt'
      ...
      for race in race_list:
         f= gene+ f"~ {race} + {snp} + PATIENT_CURRENT_AGE + YOST_INDEX_STATUS + SEX + GENE_PANEL + HAS_STAGE_IV_DX + NCI_SCORE + MOST_RECENT_INSURANCE_CATEGORY + BMI_STATUS + CANCER_TYPE" 
