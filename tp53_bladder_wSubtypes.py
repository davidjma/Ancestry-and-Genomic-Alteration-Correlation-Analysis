...
#merged_table_200=pd.merge(merged_table_200, df_text[['SAMPLE_ID','SMOKING_STATUS_PREDICTED']], left_on='SAMPLE_ID', right_on='SAMPLE_ID', how='left')
...
filename=home+'/'+'tp53_NonSmallCellLungCancer_LungAdenocarcinoma_result_summarys.txt'
gene="TP53_x"
df_final=merged_table_200
df_final=df_final.loc[df_final['CANCER_TYPE'] == 'Non-Small Cell Lung Cancer']
df_final=df_final.loc[df_final['CANCER_TYPE_DETAILED'] == 'Lung Adenocarcinoma']

print(df_final['CANCER_TYPE_DETAILED'].value_counts())
print(df_final['SAMPLE_ID'].value_counts()[:10])

df_final.drop_duplicates(subset=['SAMPLE_ID'])
print(df_final['SAMPLE_ID'].value_counts()[:10])
...
for race in race_list:
   f= gene+ f"~ {race} + PATIENT_CURRENT_AGE + YOST_INDEX_STATUS + SEX + GENE_PANEL + HAS_STAGE_IV_DX + NCI_SCORE + MOST_RECENT_INSURANCE_CATEGORY + SMOKING_STATUS_PREDICTED" 
